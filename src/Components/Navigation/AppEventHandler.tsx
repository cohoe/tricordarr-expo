...
import {getUrlForNotificationEvent} from '@tricordarr/Libraries/Notifications/SocketNotification';

// Define types for migration compatibility
type Event = any;
type EventType = any;
const EventType_DELIVERED = 'DELIVERED';
const EventType_DISMISSED = 'DISMISSED';
...
export const AppEventHandler = () => {
  ...
  // DISABLED FOR EXPO-NOTIFICATIONS MIGRATION - All event handlers disabled
  console.log('[AppEventHandler.tsx] Event handlers disabled for expo-notifications migration');
  return null;
  ...
  useEffect(() => {
    const handleForegroundEvent = async (event: Event) => {
      ...
      if (event.type === EventType_DELIVERED || event.type === EventType_DISMISSED) {
